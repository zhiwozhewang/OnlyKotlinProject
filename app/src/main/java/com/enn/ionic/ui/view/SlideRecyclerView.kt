...
class SlideRecyclerView @JvmOverloads constructor(
    ...
    override fun onInterceptTouchEvent(e: MotionEvent): Boolean {
        ...
        when (e.action) {
            MotionEvent.ACTION_DOWN -> {
                ...
                if (mPosition != SlideRecyclerView.Companion.INVALID_POSITION) {
                    ...
                    mMenuViewWidth = if (mFlingView!!.childCount == 2) {
                        ...
                        SlideRecyclerView.Companion.INVALID_CHILD_WIDTH
                    }
                }
            }
            MotionEvent.ACTION_MOVE -> {
                ...
                if (Math.abs(xVelocity) > SlideRecyclerView.Companion.SNAP_VELOCITY && Math.abs(
                        xVelocity
                    ) > Math.abs(yVelocity)
    ...
    override fun onTouchEvent(e: MotionEvent): Boolean {
        if (mIsSlide && mPosition != SlideRecyclerView.Companion.INVALID_POSITION) {
            val x = e.x
            obtainVelocity(e)
            when (e.action) {
                MotionEvent.ACTION_DOWN -> {}
                MotionEvent.ACTION_MOVE ->                     // 随手指滑动
                    if (mMenuViewWidth != SlideRecyclerView.Companion.INVALID_CHILD_WIDTH) {
                        val dx = mLastX - x
                ...
                MotionEvent.ACTION_UP -> {
                    if (mMenuViewWidth != SlideRecyclerView.Companion.INVALID_CHILD_WIDTH) {
                        ...
                        if (mVelocityTracker!!.xVelocity < -SlideRecyclerView.Companion.SNAP_VELOCITY) {    // 向左侧滑达到侧滑最低速度，则打开
                        ...
                        } else if (mVelocityTracker!!.xVelocity >= SlideRecyclerView.Companion.SNAP_VELOCITY) {  // 向右侧滑达到侧滑最低速度，则关闭
                    ...
                    mMenuViewWidth = SlideRecyclerView.Companion.INVALID_CHILD_WIDTH
                    mIsSlide = false
                    mPosition = SlideRecyclerView.Companion.INVALID_POSITION
    ...
    fun pointToPosition(x: Int, y: Int): Int {
        if (null == layoutManager) return SlideRecyclerView.Companion.INVALID_POSITION
        ...
        return SlideRecyclerView.Companion.INVALID_POSITION
    }
